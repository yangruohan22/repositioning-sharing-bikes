...
class GAOptimizer:
    def __init__(self, parameter_space: Dict[str, List[Any]], ga_params: Dict[str, Any]):
    ...
    def optimize(self, simulator: BikeSharingSimulator) -> Dict[str, Any]:
        """执行遗传算法优化"""
        # 遗传算法参数
        population_size = self.ga_params.get('population_size', 20)
        n_generations = self.ga_params.get('n_generations', 30)
        cx_prob = self.ga_params.get('crossover_prob', 0.7)
        mut_prob = self.ga_params.get('mutation_prob', 0.2)
        random_seed = self.ga_params.get('random_seed', 42)

        print(f"开始遗传算法优化，种群大小: {population_size}, 代数: {n_generations}")
        ...
        # 注册属性生成函数 - 为每个参数维度单独生成
        for i in range(len(lower_bounds)):
            toolbox.register(f"attr_float_{i}", random.uniform, lower_bounds[i], upper_bounds[i])

        # 创建个体生成器 - 使用initCycle确保正确的结构
        toolbox.register("individual", tools.initCycle, creator.Individual,
                         [toolbox.attr_float_0, toolbox.attr_float_1, toolbox.attr_float_2,
                          toolbox.attr_float_3, toolbox.attr_float_4, toolbox.attr_float_5,
                          toolbox.attr_float_6, toolbox.attr_float_7, toolbox.attr_float_8], n=1)

        # 注册种群生成函数
        toolbox.register("population", tools.initRepeat, list, toolbox.individual)

        # 注册遗传操作
        toolbox.register("evaluate", self._evaluate_individual, simulator=simulator)
        toolbox.register("mate", tools.cxBlend, alpha=0.5)
        toolbox.register("mutate", self._mutate_individual, bounds=bounds, indpb=0.2)
        ...
        self.best_value = best_individual.fitness.values[0]

        print(f"初始种群最佳适应度: {self.best_value:.2f}")
        ...
        for gen in range(n_generations):
            ...
            if current_best.fitness.values[0] > self.best_value:
                self.best_solution = current_best
                self.best_value = current_best.fitness.values[0]
            ...
            if gen % 5 == 0:  # 每5代输出一次
                print(f"第 {gen} 代: 最佳适应度 = {self.best_value:.2f}, 平均适应度 = {avg_fitness:.2f}")

        # 提取最佳参数
        best_params = self._individual_to_params(self.best_solution)
    ...
    def _evaluate_individual(self, individual, simulator):
        """评估个体的适应度"""
        # 将个体转换为策略参数
        policy_params = self._individual_to_params(individual)
        ...
        try:
            ...
            print(f"评估个体时出错: {e}")
            return (0.0,)  # 返回一个很低的适应度

    def _mutate_individual(self, individual, bounds, indpb):
        """自定义变异操作，确保参数在边界内"""
        lower_bounds, upper_bounds = bounds
        ...
        for i in range(len(individual)):
            if random.random() < indpb:
                # 高斯变异
                individual[i] += random.gauss(0, 0.1) * (upper_bounds[i] - lower_bounds[i])
                # 确保在边界内
                individual[i] = max(lower_bounds[i], min(upper_bounds[i], individual[i]))

        return individual,

    def _individual_to_params(self, individual):
        """将个体转换为策略参数"""
        return {
            'θ⁺_low': float(individual[0]),
            'θ⁺_high_gap': float(individual[1]),
            'θ⁻_low': float(individual[2]),
            'θ⁻_high_gap': float(individual[3]),
            'τ_L': int(individual[4]),  # 转换为整数
            'τ_D': int(individual[5]),  # 转换为整数
            'δ': float(individual[6]),
            'β': float(individual[7]),
            'φ': float(individual[8])
        }

    def _get_parameter_bounds(self):
        """获取参数边界"""
        # 参数顺序: θ⁺_low, θ⁺_high_gap, θ⁻_low, θ⁻_high_gap, τ_L, τ_D, δ, β, φ
        lower_bounds = [0.0, 0.0, 0.0, 0.0, 30, 30, 0.1, 0.1, 0.1]
        upper_bounds = [2.0, 2.0, 2.0, 2.0, 180, 180, 0.9, 0.9, 1.5]
...
def main():
    ...
    try:
        ...
        # 遗传算法参数 - 调整为更小的规模以便快速测试
        ga_params = {
            'population_size': 20,  # 减小种群大小
        ...
        print("开始遗传算法优化...")
        optimizer = GAOptimizer(pfa_parameter_space, ga_params)
        ...
        best_params_df.to_excel('best_pfa_parameters_ga.xlsx', index=False)
        print("最佳参数已保存到 'best_pfa_parameters_ga.xlsx'")

        # 保存优化历史
        history = optimizer.get_optimization_history()
        if history:
            history_df = pd.DataFrame(history)
            history_df.to_excel('optimization_history_ga.xlsx', index=False)
            print("优化历史已保存到 'optimization_history_ga.xlsx'")
