...
class OCBAOptimizer:
    def __init__(self, parameter_space: Dict[str, List[Any]], ocba_params: Dict[str, Any]):
        self.parameter_space = parameter_space
        self.ocba_params = ocba_params
    ...
    def generate_designs(self) -> List[Dict[str, Any]]:
        ...
        designs = []
        # 随机采样部分组合以避免组合爆炸
        n_designs = min(20, self.ocba_params.get('max_designs', 20))

        for _ in range(n_designs):
            design = {}
            for param_name, values in self.parameter_space.items():
                design[param_name] = random.choice(values)
    ...
    def optimize(self, simulator: BikeSharingSimulator) -> Dict[str, Any]:
        """执行OCBA优化"""
        n_total = self.ocba_params['N_TOTAL']
        n_init = self.ocba_params['N_INIT']
        n_add = self.ocba_params['N_ADD']

        # 初始化仿真次数
        n_replications = {i: n_init for i in range(len(self.designs))}
        results = {i: [] for i in range(len(self.designs))}

        print(f"开始OCBA优化，共有{len(self.designs)}个候选设计...")

        # 初始仿真
        for design_idx, design in enumerate(self.designs):
            print(f"初始化仿真设计 {design_idx + 1}/{len(self.designs)}")
            for rep in range(n_init):
                result_dict = simulator.simulate(design, random_seed=rep)
                # 使用需求满足量作为性能指标
                result = result_dict['total_satisfied_demand']
                results[design_idx].append(result)

        total_simulations = len(self.designs) * n_init

        # OCBA迭代
        iteration = 0
        while total_simulations < n_total:
            iteration += 1
            print(f"OCBA迭代 {iteration}, 总仿真次数: {total_simulations}/{n_total}")
            ...
            for i in range(len(self.designs)):
                if results[i]:
                    ...
                    stds[i] = np.std(results[i]) if len(results[i]) > 1 else 0.1
                else:
                    means[i] = -np.inf
                    stds[i] = 0.1

            # 找到当前最佳设计
            best_design = max(means, key=means.get)

            # 计算新的预算分配（简化版OCBA）
            new_allocations = self.calculate_allocations(means, stds, best_design,
                                                         total_simulations + n_add)

            # 执行额外仿真
            for i in range(len(self.designs)):
                additional_reps = max(0, new_allocations[i] - n_replications[i])
                if additional_reps > 0:
                    for rep in range(additional_reps):
                        result_dict = simulator.simulate(self.designs[i],
                                                         random_seed=total_simulations + rep)
                        result = result_dict['total_satisfied_demand']
                        results[i].append(result)

                n_replications[i] = len(results[i])

            total_simulations += n_add

        # 返回最佳设计
        final_means = {i: np.mean(results[i]) for i in range(len(self.designs))}
        best_design_idx = max(final_means, key=final_means.get)
        print(f"优化完成！最佳设计索引: {best_design_idx}, 性能: {final_means[best_design_idx]:.2f}")
        return self.designs[best_design_idx]

    def calculate_allocations(self, means: Dict[int, float], stds: Dict[int, float],
                              best_design: int, total_budget: int) -> Dict[int, int]:
        """计算OCBA预算分配（简化版本）"""
        allocations = {}
        remaining_budget = total_budget

        # 给最佳设计分配更多预算
        allocations[best_design] = int(total_budget * 0.4)
        remaining_budget -= allocations[best_design]

        # 其余设计平均分配剩余预算
        other_designs = [i for i in range(len(self.designs)) if i != best_design]
        if other_designs:
            budget_per_design = remaining_budget // len(other_designs)
            for i in other_designs:
                allocations[i] = budget_per_design
...
def main():
    ...
    try:
        ...
        # OCBA参数（根据计算资源调整）
        ocba_params = {
            'N_TOTAL': 5000,  # 总仿真次数（测试用，可增加）
            'N_INIT': 3,  # 初始仿真次数
            'N_ADD': 20,  # 每次迭代增加次数
            'max_designs': 10  # 最大设计数
        }
        ...
        print("开始OCBA优化...")
        optimizer = OCBAOptimizer(pfa_parameter_space, ocba_params)
        ...
        best_params_df.to_excel('best_pfa_parameters.xlsx', index=False)
        print("最佳参数已保存到 'best_pfa_parameters.xlsx'")
